...
async function fetchManagedLabs(assistantId) {
    try {
        const labsQuery = query(collection(db, 'labs'), where('assistantId', '==', assistantId));
...
function showSection(sectionName) {
    ...
    switch(sectionName) {
        case 'labs':
            setupLabsListener(auth.currentUser.uid);
            break;
        case 'requests':
...
function setupLabsListener(assistantId) {
    // This listener is now implicitly handled by fetchManagedLabs and re-rendering on data change elsewhere
    labsList.innerHTML = '';
...
function openRegistersModal(lab) {
         ...
         <div class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center modal z-50">
            <div class="bg-white p-8 rounded-lg shadow-xl max-w-md w-full relative">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                    <button class="reg-nav-btn bg-blue-500 text-white py-3 rounded-lg font-semibold hover:bg-blue-600" data-page="deadstock_register">Dead Stock</button>
                    <button class="reg-nav-btn bg-blue-500 text-white py-3 rounded-lg font-semibold hover:bg-blue-600" data-page="issue_register">Issue</button>
                    <button class="reg-nav-btn bg-blue-500 text-white py-3 rounded-lg font-semibold hover:bg-blue-600" data-page="Consumable_register">Consumable</button>
                    <button class="reg-nav-btn bg-blue-500 text-white py-3 rounded-lg font-semibold hover:bg-blue-600" data-page="maintenance_register">Maintenance</button>
                </div>
            </div>
        </div>
    `;
    ...
    document.querySelectorAll('.reg-nav-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            window.location.href = `${btn.dataset.page}.html?labId=${lab.id}&dashboard=assistant_dashboard`;
        });
    });
}
